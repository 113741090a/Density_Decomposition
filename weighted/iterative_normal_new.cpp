...
void update_density(optim *opt){
  ...
  for (int i=0; i < opt->n; i++){
    opt->hypernodes[i].rho = opt->hypernodes[i].r / opt->hypernodes[i].weight;
  }
}
...
double multiplicative(optim *opt){
  double res= 0.0;
  for (int i=0; i < opt->n; i++){
    double error = (opt->hypernodes[i].rho - opt->hypernodes[i].rho_correct) / (opt->hypernodes[i].rho_correct);
    if (error < 0) error = -error;
    if (error > res) {
      res = error;
      //printf("res error is = %lf, idx = %d, approximate rho %lf, exact rho %lf\n",res, i, opt->hypernodes[i].rho, opt->hypernodes[i].rho_correct);
    }
  }
  return res;
}
...
long long merge(node* Mark_nodes, int l, int mid, int r){
  ...
  for (int i = 0; i < mid - l + 1; i++) temp_arr[i].rho_correct = Mark_nodes[i+l].rho_correct;
  int i1=0;
  int i2 = mid+1;
  int j=l;
  while (i1 <mid - l + 1 && i2<=r){
    if (temp_arr[i1].rho_correct <= Mark_nodes[i2].rho_correct){
...
int main(int argc,char** argv){
  ...
  double rho = 0.0;
    ...
    opt->hypernodes[id].rho_correct = rho;
  ...
  int flag_increase = 1;
  vector<string> algos = {"PR_exp", "FISTA*", "PR", "PR_lin"};
...
//--------------------specical algorithms---------------------------------------------------
  ...
  //printf("Current time: %ld seconds and %ld microseconds\n", currentTime.tv_sec, currentTime.tv_usec);
    ...
    if ((i+1)%num_points == 0){
      ...
      for (int i=0; i < opt->n; i++){
        Mark_nodes[i].rho = Mark_nodes[i].r / Mark_nodes[i].weight;
      }
      qsort(Mark_nodes,opt->n,sizeof(node),compare_nodes2);
      inv = num_inversions(Mark_nodes, 0, opt->n-1);
      inv_arr[i] = inv;
