...
void read_weights(optim *opt, char *name1, char *name2){
  ...
  double weight;
	...
	fclose(file);
	file=fopen(name2,"r");
    printf("start to read nodes\n");
	while (fscanf(file,"%d %lf", &(idx), &(weight))==2) {
    //opt->edges[opt->e].s = 
        //printf("new idx start to read\n");
		int new_idx = opt->newlabel2[idx];
...
int main(int argc,char** argv){
  ...
  printf("Memory usage: %ld GB\n", memory_usage_mb);
  //read weights
  printf("start to read weights\n");
  read_weights(opt, "bipartite_mark_E.txt", "bipartite_mark_N.txt");
  printf("read weights finished\n");
  //check weights are read correctly. 
  for (int i =0; i < opt->m; i++){
    if (opt->hyperedges[i].weight <= 0) printf("mistake\n");
  }
  for (int i =0; i < opt->n; i++){
    if (opt->hypernodes[i].weight <= 0) printf("mistake\n");
  }
