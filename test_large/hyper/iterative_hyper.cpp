...
int main(int argc,char** argv){
  ...
  //isoreg *fit,*fit2;
	...
	//unsigned rep=1200;
  unsigned rep = 200;
  char* edgelist="bipartite_hyper.txt";
  if (argc > 2){
    edgelist = argv[1];
    rep = atoi(argv[2]);
  }
  ...
  FILE *exact = fopen("Exact_hyper.txt", "r");
  int id = 0;
  double rho = 0.0;
  ...
  for (i=0; i < rep; i++) simulation[i] = (double*)malloc(4*sizeof(double));
  ...
  //vector<string> algos = {"FISTA", "FISTA*", "PR", "PR_lin", "PR_exp"};
  //vector<string> algos = {"FISTA"};
  //vector<string> algos = {"FISTA", "FISTA*", "PR", "PR_lin", "PR_exp"};

  //vector<string> algos = {}; , "FISTA*", , "PR", "PR_lin"
  vector<string> algos = {"FISTA", "PR_exp" };
  // Iterate over the vector of algorithms 
