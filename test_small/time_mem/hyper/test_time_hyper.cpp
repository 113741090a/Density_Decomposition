...
void quicksort(int arr[], int low, int high, double* edges);
...
void onepass_PR_plus_product_hyper(optim *opt, double* last_x, double* z){
	int i,j;
	unsigned long long k;
...
//be carefull here. 
  ...
  for (i=0; i < opt->m; i++){
    ...
    for (int i1 = 0; i1 < opt->hyperedges[i].degree; i1++){
      unsigned edge_idx = opt->hyperedges[i].neighboredges[i1];
      total_weight = total_weight + z[edge_idx];
    }
    double parameter = hyperedge_weight / (total_weight);
    for (int i1 = 0; i1 < opt->hyperedges[i].degree; i1++){
      unsigned edge_idx = opt->hyperedges[i].neighboredges[i1];
      z[edge_idx] = z[edge_idx] * parameter;
    }
  }
}


void onepass_PR_plus_lin_hyper(optim *opt, double* last_x, double* z){
	int i,j;
	unsigned long long k;
...
//be carefull here. 
  ...
  //do projection on z. from a non-feasible z to z. 
  //#pragma omp parallel for private(i,j,k)
  for (i=0; i < opt->m; i++){
    ...
    quicksort(opt->hyperedges[i].neighboredges, 0, opt->hyperedges[i].degree-1, z);
    int rho = 0;
    double weight = opt->hyperedges[i].weight;
    double total_rho = 0.0;
    for (j=opt->hyperedges[i].degree-1; j >=0; j--){
      //printf("start to do quicksort, i, j =%d, %d\n",i, j);
      unsigned edge_idx = opt->hyperedges[i].neighboredges[j];
      double edge_weight = z[edge_idx];
      
      double value = edge_weight + 1.0/(rho+1) * (weight - total_rho-edge_weight); //change to edge weight..., now it's 1
      if (value <= 0) break;
      total_rho = total_rho + edge_weight;
      rho++;
    }
    //printf("\n");
    double lambda = 1.0 / (rho) * (weight - total_rho);
    for (j=opt->hyperedges[i].degree-1; j >=0; j--){
      unsigned edge_idx = opt->hyperedges[i].neighboredges[j];
      z[edge_idx] = z[edge_idx] + lambda;
      if (z[edge_idx] < 0) z[edge_idx] = 0;
      //printf("%.20f, ", opt->edges[edge_idx].a);
    }
    //printf("\n");
  }
  //*lambda = lambda1;
...
int partition(int arr[], int low, int high, double* edge_weights) {
    int pivot_idx = arr[high];
    double pivot_value = edge_weights[pivot_idx];
    int i = (low - 1);

    for (int j = low; j <= high - 1; j++) {
      double j_value = edge_weights[arr[j]];
      if (j_value < pivot_value) {
          i++;
          swap(&arr[i], &arr[j]);
      }
...
void quicksort(int arr[], int low, int high, double* edge_weights) {
    if (low < high) {
        int pi = partition(arr, low, high, edge_weights);
        quicksort(arr, low, pi - 1, edge_weights);
        quicksort(arr, pi + 1, high, edge_weights);
    }
}
...
//one pass over all edges
void onepass_FISTA(optim *opt, double max_degree, double* z, double* y, double* last_x){
	int i,j;
	unsigned long long k;
	opt->iter++;
  //in their own implementation, perhaps it is 0.9/Delta
  double alpha = 0.5 / (max_degree);
  double payload = 0;
  ...
  for (i=0; i < opt->m; i++){
    ...
    for (j=opt->hyperedges[i].degree-1; j >=0; j--){
      unsigned edge_idx = opt->hyperedges[i].neighboredges[j];
      opt->edges[edge_idx].a = z[edge_idx] + lambda;
      if (opt->edges[edge_idx].a < 0) opt->edges[edge_idx].a = 0;
      //printf("%.20f, ", opt->edges[edge_idx].a);
    }
    //printf("\n");
  }
  ...
  //     for (j=0; j < opt->hyperedges[i].degree; j++){
  //       int edge_idx = opt->hyperedges[i].neighboredges[j];
  //       total = total + opt->edges[edge_idx].a;
  //     }
  //     printf("total = %lf\n", total);
  // }
  for (k = 0; k < opt->e; k++){
...
//one pass over all edges
void onepass_FISTA_star(optim *opt, double max_degree, double* z, double* y, double* last_x){
	int i,j;
	unsigned long long k;
	opt->iter++;
  //in their own implementation, perhaps it is 0.9/Delta
  double alpha = 0.9 / (max_degree);
  double payload = 0;
  update_density2(opt, y);
...
double calculateMean(long int* arr, int size) {
    long int sum = 0;
...
double calculateStandardDeviation(long int* arr, int size) {
...
long getMemoryUsage() {
    ...
    return usage.ru_maxrss; // 返回最大常驻内存集，以KB为单位
}
...
long getMaxMemoryUsage() {
    ...
    while (fgets(line, sizeof(line), file)) {
        if (strncmp(line, "VmPeak:", 7) == 0) {
            sscanf(line + 7, "%ld", &memoryUsage); // 解析内存使用值
            break;
        }
    }

    fclose(file);
    return memoryUsage; // 返回峰值虚拟内存，以KB为单位
}
...
int main(int argc,char** argv){
  ...
  double *z = (double*)malloc(opt->e*sizeof(double));
  double *y = (double*)malloc(opt->e*sizeof(double));
  double *last_x = (double*)malloc(opt->e*sizeof(double));
  //double *last_gradient = (double*)malloc(opt->e*sizeof(double));
	for (i = 0; i < opt->e; i++){
  ...
  long int* time_arr = (long int*)malloc(10*sizeof(long int));
  int flag_increase = 1;
  vector<string> algos = {"PR_exp", "FISTA*",  "Elist++", "PR", "PR_lin", "Greedy++",}; //   , "PR", "PR_lin", "Greedy++"
  //vector<string> algos = {"PR_exp", "FISTA*",  "Elist++"}; //"PR", "PR_lin", "Greedy++",
  rep = 100;
  string folder_name = "data_hyper/";
  string file_name_cpp = "";
  const char* file_name_c;
  string time_name = "_time.txt";
  FILE *file_time;
  // Iterate over the vector of algorithms "FISTA", , "PR_exp" "FISTA*",  , "FISTA*", "PR", "PR_lin"
  for (const std::string& algo_name : algos) {
    ...
    for (int i1 = 0; i1<10; i1++){
      ...
      for (i=0;i<rep;i++){
        ...
        if (algo_name == "FISTA") onepass_FISTA(opt, max_degree, z, y, last_x);
        else if (algo_name == "PR") onepass_PR(opt);
        else if (algo_name == "PR_lin") onepass_PR_plus_lin_hyper(opt, last_x, z);
        else if (algo_name == "PR_exp") onepass_PR_plus_product_hyper(opt, last_x, z);
        else if (algo_name == "FISTA*") onepass_FISTA_star(opt, max_degree, z, y, last_x);
        else if (algo_name == "Greedy++") onepass_Greedypp(opt);
        else if (algo_name == "Elist++") onepass_Elistpp(opt);
      ...
      time_arr[i1] = total_time;
    }
    double average = calculateMean(time_arr, 10);
    double std_mark = calculateStandardDeviation(time_arr, 10);
    file_name_cpp = "hyper/"+folder_name + algo_name + time_name;
    file_name_c = file_name_cpp.c_str();
    file_time = fopen(file_name_c, "a");
    memory = getMaxMemoryUsage();
    fprintf(file_time, "%lf %lf %ld %llu\n", average, std_mark, memory, opt->e);
    fclose(file_time);
