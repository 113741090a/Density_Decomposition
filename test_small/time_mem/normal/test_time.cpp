...
int main(int argc,char** argv){
  ...
  double max_degree = get_max_weighted_degree(opt);
	...
	double *z1; 
	z1 = (double*)malloc(opt->e*sizeof(double));
	double *y1; 
	y1 = (double*)malloc(opt->e*sizeof(double));
	double *last_x1;
	last_x1 = (double*)malloc(opt->e*sizeof(double));
	for (i = 0; i < opt->e; i++){
		// z[i].s = opt->edges[i].s;
		// z[i].t = opt->edges[i].t;
		z1[i] = opt->edges[i].a;
  ...
  for (const std::string& algo_name : algos) {
    ...
    if (flag_init == 1){
      ...
      for (int i1 = 0; i1 < opt->e; i1++){
        last_x1[i1] = opt->edges[i1].a;
        z1[i1] = opt->edges[i1].a;
        //last_x[i].b = 1 - opt->edges[i].a;
      }
    }
    //如果是fista，计算。
    ...
    for (int i1 = 0; i1<10; i1++){
      ...
      for (i=0;i<rep;i++){
        ...
        else if (algo_name == "FISTA") onepass_FISTA_normal(opt, max_degree, z1, y1, last_x1);
        else if (algo_name == "FW_Elist") onepass_FW_Elist(opt);
        else if (algo_name == "PR") onepass_PR(opt);
        else if (algo_name == "PR_lin") onepass_PR_plus_lin_normal(opt, last_x1, z1);
        else if (algo_name == "PR_exp") onepass_PR_plus_product_normal_pro(opt, last_x1, z1);
        else if (algo_name == "FISTA*") onepass_FISTA_star_normal(opt, max_degree, z1, y1, last_x1);
...
void onepass_PR_plus_product_normal(optim *opt, link0* last_x, double* z){
	int i,j;
	unsigned long long k;
