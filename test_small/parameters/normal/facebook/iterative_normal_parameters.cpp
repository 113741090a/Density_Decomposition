...
//be carefull here. 
  ...
  //double gamma = (double)(t-1)/(double)(t+2);
  //double gamma = (double)(t-1)/(double)(t+2);
  double parameter_mark = 3.0;
  double gamma = 1 - (double) parameter_mark/(t+2);
  for (k = 0; k < opt->e; k++){
    //printf("values %lf, %lf\n", opt->edges[k].a, last_x[k].a);
    if (opt->edges[k].a == 0.0 || last_x[k].a == 0.0) z[k].a = 0.0;
    else z[k].a = pow(opt->edges[k].a, 1+gamma) * pow(last_x[k].a, -gamma);
...
//proportional projection
void onepass_PR_plus_product_normal_pro1(optim *opt, link0* last_x, link0* z){
	int i,j;
	unsigned long long k;
...
//proportional projection
void onepass_PR_plus_product_normal_pro2(optim *opt, link0* last_x, link0* z){
	int i,j;
	unsigned long long k;
...
//proportional projection
void onepass_PR_plus_product_normal_pro4(optim *opt, link0* last_x, link0* z){
	int i,j;
	unsigned long long k;
...
//proportional projection
void onepass_PR_plus_product_normal_pro5(optim *opt, link0* last_x, link0* z){
	int i,j;
	unsigned long long k;
...
//proportional projection
void onepass_PR_plus_product_normal_pro6(optim *opt, link0* last_x, link0* z){
	int i,j;
	unsigned long long k;
...
//proportional projection
void onepass_PR_plus_product_normal_pro_para(optim *opt, link0* last_x, link0* z, double parameter_mark){
	int i,j;
	unsigned long long k;
...
int main(int argc,char** argv){
  ...
  vector<string> algos = {"PR_exp1", "PR_exp2", "PR_exp3", "PR_exp4", "PR_exp5", "PR_exp6"};
  // Iterate over the vector of algorithms "FISTA", "FISTA*", "PR_exp" "FISTA*", 
  for (const std::string& algo_name : algos) {
    ...
    for (i=0;i<rep;i++){
      ...
      char lastChar = algo_name.back(); // 获取字符串最后一个字符
      int lastDigit = lastChar - '0'; // 将字符转换为整数
      onepass_PR_plus_product_normal_pro_para(opt, last_x, z, (double) lastDigit);
      ...
      // if (algo_name == "PR_exp1") onepass_PR_plus_product_normal_pro1(opt, last_x, z);
      // else if (algo_name == "PR_exp2") onepass_PR_plus_product_normal_pro2(opt, last_x, z);
      // else if (algo_name == "PR_exp3") onepass_PR_plus_product_normal_pro(opt, last_x, z);
      // else if (algo_name == "PR_exp4") onepass_PR_plus_product_normal_pro4(opt, last_x, z);
      // else if (algo_name == "PR_exp5") onepass_PR_plus_product_normal_pro5(opt, last_x, z);
      // else if (algo_name == "PR_exp6") onepass_PR_plus_product_normal_pro6(opt, last_x, z);
